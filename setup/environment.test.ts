...
describe('credentials detection', () => {
  it('detects OpenAI API key in env content', () => {
    const content = 'SOME_KEY=value\nOPENAI_API_KEY=sk-test123\nOTHER=foo';
    const hasCredentials = /^OPENAI_API_KEY=/m.test(content);
    expect(hasCredentials).toBe(true);
  });

  it('detects Codex auth token in env content', () => {
    const content = 'CODEX_AUTH_TOKEN=token123';
    const hasCredentials = /^CODEX_AUTH_TOKEN=/m.test(content);
    expect(hasCredentials).toBe(true);
  });

  it('returns false when no credentials', () => {
    const content = 'ASSISTANT_NAME="Andy"\nOTHER=foo';
    const hasCredentials = /^(OPENAI_API_KEY|CODEX_AUTH_TOKEN)=/m.test(content);
